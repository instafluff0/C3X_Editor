...
CivAssist II's Economy screen is partly a hypothetical simulator. Civ Advisor's main Economy values intentionally show the saved Domestic Advisor economy. The supported what-ifs are the structured Government picker plus Science/Luxury rate sliders. Government preview recalculates unit support exactly from the save-native `GOVT` rules, current cities, and current owned units. City corruption and shield waste previews now follow Civ3's `City::compute_corrupted_yield` structure: government corruption type, map distance/rank, world-size OCN, commercial/AI/difficulty OCN modifiers, courthouse/police-style corruption reducers, palace and forbidden-palace/wonder pseudo-capital effects, communal rank handling, difficulty corruption scaling, specialist corruption reduction, and final caps. The current implementation keeps a trade-connection hook but defaults cities to connected until the full Civ3 trade-net path is available in the report layer. Rate preview infers each city's pre-multiplier slider commerce from saved `Science`/`Luxury`/`AddCash` fields, then applies Civ3's `((rate_step * commerce) + 5) / 10` city allocation and the saved city's science/luxury/tax building multipliers. If several raw-commerce values reproduce the saved city output exactly, use the midpoint of that exact-match range; choosing the endpoint overfits the current slider and produces worse alternate-rate projections. Shield preview similarly infers raw unwasted production before factory/power multipliers; Civ3 computes waste against raw tile production first and applies production multipliers after waste. Specialist `AddLuxury`/`AddScience`/`AddTaxes` remain separate. When a simulation is active, the existing Economy values switch to the simulated context and show red/green deltas against the saved values. Government and rate simulations are identified in the relevant section headers rather than only inline next to one control.
...
Government preview should remain scoped to the currently selected save/player and reset when either changes. It should use the save's embedded government records rather than a hardcoded stock Civ3 government list, so modded government names and support rules are preserved. City-level government preview remains an estimate until the report layer can rebuild the exact worked-tile yields and trade-network capital connectivity. The corruption/waste step itself is now source-shaped, but full government-preview parity still depends on tile-by-tile food/shield/commerce recomputation, government tile penalty/commerce bonus on actual worked tiles, exact Civ3 trade-net connectivity, and C3X options.

C3X keeps the base Civ3 commerce allocation but patches the inputs and outputs that feed it. Relevant hooks in `injected_code.c` include expanded city work areas (`patch_City_controls_tile` and `patch_City_*_working_tile_conv_ni`), district tile yield replacement (`patch_City_calc_tile_yield_while_gathering`), district/distribution-hub city-center bonuses, district science added after `City_recompute_commerce`, and `patch_City_compute_corrupted_yield` for C3X corruption/decorruption options. Exact C3X parity therefore requires the loaded C3X config snapshot plus the save's C3X district instance segment; do not infer district yield effects from the `.SAV` aggregate values and then add them again.
