...
test('renderer exposes the Units-tab FLC Workshop entry point', () => {
  ...
  assert.match(renderer, /loadFlicWorkshopAnimation\(next, \{ resetStatus: true \}\)/);
...
test('FLC Workshop action selector and palette grid stay compact', () => {
  ...
  const controlGroupRule = css.match(/\.flic-workshop-control-group\s*\{[^}]+\}/);
  ...
  assert.match(exportChoiceRule[0], /grid-template-columns:\s*minmax\(220px,\s*1fr\);/);
